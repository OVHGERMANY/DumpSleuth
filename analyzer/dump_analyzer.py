...
class DumpAnalyzer:
    ...
    def basic_file_info(self) -> Dict[str, Any]:
        ...
        if len(filename_parts) >= 3:
            ...
            file_info["crash_date"] = (
                filename_parts[1] if len(filename_parts) > 1 else "Unknown"
            )
            file_info["crash_time"] = (
    ...
    def extract_strings(
        self, min_length: int = 4, max_strings: int = 1000
    ) -> Dict[str, Any]:
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            for section, data in self.results.items():
                ...
                if section == "Extracted Strings" and isinstance(data, dict):
                    ...
                    f.write(
                        f"**Total Strings Found**: {data.get('total_count', 0)}\n\n"
                    )
                    ...
                    if section == "Pattern Matches":
                        for pattern_type, matches in data.items():
                            if matches:
                                f.write(
                                    f"### {pattern_type} ({len(matches)} found)\n\n"
                                )

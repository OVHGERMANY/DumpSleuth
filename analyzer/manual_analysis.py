...
class ManualAnalyzer:
    def __init__(self, dump_file: str):
        ...
        self.console.print(
            f"[green]📁 Loaded: {dump_file} ({self.file_size:,} bytes)[/green]"
        )
    ...
    def show_file_info(self):
        ...
        table.add_row(
            "Size", f"{self.file_size:,} bytes ({self.file_size / (1024*1024):.2f} MB)"
        )
        table.add_row("Created", str(stat.st_ctime))
        table.add_row("Modified", str(stat.st_mtime))
        table.add_row(
            "Current Offset", f"0x{self.current_offset:08X} ({self.current_offset:,})"
        )
    ...
    def hex_viewer(self):
        ...
        while True:
            ...
            # Display hex dump
            self.console.print(
                Panel(hexdump.hexdump(data, result="return"), title="Hex Data")
            )
            ...
            if cmd in ["n", "next"]:
                self.current_offset = min(
                    self.current_offset + bytes_per_page, self.file_size - 1
                )
            elif cmd in ["p", "prev"]:
                ...
                self.console.print(
                    f"[green]✅ Bookmark '{name}' added at 0x{self.current_offset:08X}[/green]"
                )
            elif cmd in ["q", "quit"]:
                break
    ...
    def search_bytes(self):
        ...
        pattern_str = Prompt.ask(
            "Enter search pattern (hex bytes, e.g., '41 42 43' or 'ABC')"
        )
        ...
        try:
            ...
            self.console.print(
                f"[cyan]🔍 Searching for pattern: {pattern.hex().upper()}[/cyan]"
            )
    ...
    def string_search(self):
        ...
        self.console.print(
            f"[cyan]🔤 Searching for strings containing '{search_term}'...[/cyan]"
        )
        ...
        try:
            ...
            if matches:
                self.console.print(
                    f"[green]✅ Found {len(matches)} matching strings:[/green]"
                )
                table = Table()
                ...
                for offset, string in matches[:20]:
                    table.add_row(
                        f"0x{offset:08X}",
                        string[:80] + ("..." if len(string) > 80 else ""),
                    )
    ...
    def jump_to_offset(self):
        ...
        try:
            ...
            if 0 <= offset < self.file_size:
                ...
                self.console.print(
                    f"[red]❌ Offset out of range (0 - 0x{self.file_size-1:08X})[/red]"
                )
        except ValueError:
            self.console.print("[red]❌ Invalid offset format[/red]")
    ...
    def bookmarks_menu(self):
        ...
        while True:
            ...
            if choice == "1":
                ...
                self.bookmarks[name] = self.current_offset
                self.console.print(
                    f"[green]✅ Added bookmark '{name}' at 0x{self.current_offset:08X}[/green]"
                )
            elif choice == "2":
                if self.bookmarks:
                    name = Prompt.ask(
                        "Bookmark name", choices=list(self.bookmarks.keys())
                    )
                ...
                if self.bookmarks:
                    name = Prompt.ask(
                        "Bookmark to delete", choices=list(self.bookmarks.keys())
                    )
    ...
    def export_current_view(self):
        ...
        self.console.print(
            f"[green]✅ Exported {len(data)} bytes to {filename}[/green]"
        )
    ...
    def export_strings(self):
        ...
        self.console.print(
            f"[green]✅ Exported {len(strings)} strings to {filename}[/green]"
        )

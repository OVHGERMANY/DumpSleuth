...
class StringExtractor:
    ...
    def filter_strings(self, strings: List[str], filters: Dict[str, str]) -> Dict[str, List[str]]:
    ...
    def display_results(self, categorized_strings: Dict[str, List[str]]):
        ...
        for category, strings in categorized_strings.items():
            ...
            for string in strings[:20]:
                table.add_row(string[:100] + ("..." if len(string) > 100 else ""), str(len(string)))
    ...
    def extract_from_file(self, file_path: str, min_length: int = 4, max_size_mb: int = 50) -> Dict[str, List[str]]:
        """Extract strings from a file"""
        self.console.print(f"[bold cyan]🔍 Extracting strings from: {file_path}[/bold cyan]")
        
        try:
            file_size = Path(file_path).stat().st_size
            
            if file_size > max_size_mb * 1024 * 1024:
                self.console.print(f"[yellow]⚠️  Large file ({file_size // (1024*1024)}MB). Reading first {max_size_mb}MB only.[/yellow]")
            ...
            self.console.print(f"[green]✅ Found {len(all_strings)} unique strings[/green]")
    ...
    def search_strings(self, categorized_strings: Dict[str, List[str]], search_term: str) -> List[str]:
...
def main():
    parser = argparse.ArgumentParser(description="Extract and analyze strings from dump files")
    parser.add_argument("file", nargs="?", help="Dump file to analyze")
    parser.add_argument("-l", "--min-length", type=int, default=4, help="Minimum string length (default: 4)")
    parser.add_argument("-o", "--output", help="Output file for results")
    parser.add_argument("-s", "--search", help="Search for specific term in results")
    parser.add_argument("--max-size", type=int, default=50, help="Maximum file size to read in MB (default: 50)")
    ...
    if args.file:
        ...
        if not dump_files:
            print("❌ No .dmp files found! Specify a file or put a .dmp file in current directory.")
            sys.exit(1)
    ...
    # Extract strings
    categorized_strings = extractor.extract_from_file(dump_file, args.min_length, args.max_size)

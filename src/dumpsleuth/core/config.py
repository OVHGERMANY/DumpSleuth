...
class ConfigManager:
    ...
    def _update_config(self, data: Dict[str, Any]) -> None:
        ...
        if "analysis" in data:
            ...
            if "strings" in analysis_data:
                ...
                self.config.analysis.string_min_length = string_config.get(
                    "min_length", 4
                )
                self.config.analysis.string_max_length = string_config.get(
                    "max_length", 256
                )
                self.config.analysis.encoding = string_config.get(
                    "encoding", ["utf-8", "utf-16", "ascii"]
                )

            self.config.analysis.max_file_size = analysis_data.get(
                "max_file_size", 1024
            )
            self.config.analysis.parallel_processing = analysis_data.get(
        ...
        if "output" in data:
            ...
            self.config.output.include_raw_data = output_data.get(
    ...
    def validate_config(self) -> List[str]:
        ...
        if self.config.analysis.max_file_size > 2048:
            warnings.append(
                "Max file size is very large (>2GB), this may cause memory issues"
            )
        ...
        if self.config.analysis.worker_threads > cpu_count * 2:
            warnings.append(
                f"Worker threads ({self.config.analysis.worker_threads}) exceeds 2x CPU count ({cpu_count})"
            )

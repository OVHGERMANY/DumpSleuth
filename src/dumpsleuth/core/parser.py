...
class DumpParser:
    ...
    def __init__(self, dump_file: Union[str, Path], config: Config):
        ...
        if self.file_size > max_size:
            raise ValueError(
                f"Dump file too large: {self.file_size:,} bytes "
        ...
        try:
            ...
            if use_mmap and self.file_size > mmap_threshold:
                try:
                    ...
                    logger.warning(
                        f"Failed to create mmap: {e}, using regular file access"
                    )
    ...
    def _parse_windows_dump_header(self, dump_data: DumpData):
        ...
        if b"Windows" in header_data:
            ...
            if end > start:
                dump_data.metadata["os_version"] = header_data[start:end].decode(
                    "utf-8", errors="ignore"
                )

...
class ProcessExtractorPlugin(AnalyzerPlugin):
    ...
    def _parse_eprocess(self, dump_data: DumpData, offset: int) -> Optional[ProcessInfo]:
        ...
        while offset < file_size:
            ...
            for match in self.signatures['linux']['comm_pattern'].finditer(chunk):
                ...
                if (name and 
                    ...
                    '.' not in name[-4:]):  # Not a file extension
    ...
    def _generate_summary(self, processes: List[ProcessInfo]) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_system_processes': any(
                'system' in p.name.lower() or 
                'kernel' in p.name.lower() or
                'init' in p.name.lower()
                for p in processes if p.name
            )
        } 
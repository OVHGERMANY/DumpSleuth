...
class StringExtractor(AnalyzerPlugin):
    ...
    def _find_patterns(self, strings: List[str]) -> Dict[str, List[str]]:
        ...
        for string in strings:
            ...
            patterns_found["bitcoin_addresses"].extend(re.findall(bitcoin_pattern, string))
        ...
        for key in patterns_found:
            if key == "hashes":
                for hash_type in patterns_found["hashes"]:
                    patterns_found["hashes"][hash_type] = list(set(patterns_found["hashes"][hash_type]))[:50]
            else:
    ...
    def _calculate_statistics(self, categorized: Dict[str, List[str]], 

...
"""
    ...
    def _format_nav(self, data: Dict[str, Any]) -> str:
        ...
        for item in nav_items:
            ...
            pills.append(f'<a class="nav-pill" onclick="showSection(\'{item}\')">{label}</a>')
    ...
    def _format_summary_card(self, summary: Dict[str, Any]) -> str:
        ...
        for key, value in summary.items():
            ...
            if isinstance(value, bool):
                icon = "✅" if value else "❌"
                items.append(f"<li>{icon} <strong>{formatted_key}:</strong> {formatted_value}</li>")
            else:
                items.append(f"<li><strong>{formatted_key}:</strong> {formatted_value}</li>")
    ...
    def _format_list_card(self, title: str, items: List[Any]) -> str:
        ...
        if isinstance(items[0], dict):
            ...
            for item in items[:100]:  # Limit to first 100 items
                cells = ''.join(
                    f'<td>{html.escape(str(item.get(h, "")))}</td>' 
                    for h in headers
                )
